    ...
    <SyncContainer withPulse={true} className="overflow-hidden border-cyan-500/10">
      ...
      >
        {parsedStats.map(({ stat, parsed }, index) => (
          <div
            key={stat.label}
            className="group relative bg-[#04090d]/80 px-6 py-10 backdrop-blur transition-all duration-500 hover:bg-[#04090d]/40"
          >
            <DataPulse className="opacity-0 group-hover:opacity-40 transition-opacity" />
            <div className="absolute inset-x-0 top-0 h-px origin-center scale-x-0 bg-gradient-to-r from-cyan-400 via-violet-400 to-cyan-400 transition-transform duration-500 group-hover:scale-x-100" aria-hidden="true" />
            ...
            <dt className="relative z-10">
              <GlitchText trigger="hover" intensity="low">
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 transition-colors group-hover:text-cyan-400/70">
                  {stat.label}
                </span>
              </GlitchText>
            </dt>
            <dd className="relative z-10 mt-4 text-4xl font-black tracking-tight text-white transition-[filter] duration-500 group-hover:drop-shadow-[0_0_15px_rgba(34,211,238,0.5)] sm:text-5xl tabular-nums">
              {parsed.isAnimatable ? (
                <AnimatedNumber
                  value={parsed.number}
