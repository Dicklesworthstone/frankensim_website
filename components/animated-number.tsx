...
// A CSS-independent screen-reader-only style. The visible number is animated
// while a static copy carries the final value for assistive tech. Hiding that
// static copy must NOT depend on a purgeable `.sr-only` utility being present
// in the production stylesheet — if it were missing, both copies would paint
// and the number would render doubled ("54" → "5454"). Inline styles guarantee
// the static copy is always visually hidden, so the value shows exactly once.
...
export function AnimatedNumber({
  ...
  return (
    <span className={className}>
      <span className="sr-only" style={VISUALLY_HIDDEN}>
        {prefix}{srValue}{suffix}
      </span>
