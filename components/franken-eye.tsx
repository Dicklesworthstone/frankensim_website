...
/**
 * The creation, watching. A reactive monster eye that tracks the cursor,
 * blinks, dilates on hover, and flushes its vessels as you approach.
 */
export default function FrankenEye({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Blood vessels — flush on proximity */}
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <motion.div
          className="relative h-6 w-6 rounded-full bg-cyan-500 border border-cyan-700 flex items-center justify-center shadow-[inset_0_0_10px_rgba(0,0,0,0.3)]"
          style={{ x: mousePos.x, y: mousePos.y }}
          transition={{ type: "spring", stiffness: 250, damping: 20 }}
        >
          <div className="absolute inset-0 bg-[repeating-conic-gradient(from_0deg,_transparent_0deg_10deg,_rgba(0,0,0,0.12)_10deg_20deg)] opacity-40 rounded-full" />
          <motion.div
            className="h-3 w-3 rounded-full bg-slate-950"
            animate={{ scale: isHovered ? 1.25 : 1, backgroundColor: isHovered ? "#000" : "#020617" }}
          />
